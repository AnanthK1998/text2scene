...
def get_scene_list(poses, scenes):
    ...
    for scene in scenes:
        ...
        for obj in scene:
            ...
            poses[outer_count][inner_count][0:3] = torch.tensor(np.dot(poses[outer_count][inner_count][0:3].cpu().numpy(), np.transpose(rot_mat.cpu().numpy())))
            ...
            obj_points = tmp.verts_packed().cpu().numpy()
            ...
            obj_points = torch.tensor(obj_points.dot(np.diag(1/(obj_points.max(0) - obj_points.min(0)))).dot(np.diag(poses[outer_count][inner_count][3:6].cpu().numpy())))
            ...
            obj_points = np.array(obj_points.cpu().numpy().dot(axis_rectified.cpu().numpy())) + poses[outer_count][inner_count][0:3].cpu().numpy()

            verts_rgb = torch.ones_like(torch.tensor(obj_points),dtype= torch.float32)[None]  # (1, V, 3)
            textures = TexturesVertex(verts_features=verts_rgb.to(device))

            obj_list.append(Meshes(verts=[torch.tensor(obj_points,dtype=torch.float32).to(device)],faces=[tmp.faces_packed().to(device)],textures=textures))
